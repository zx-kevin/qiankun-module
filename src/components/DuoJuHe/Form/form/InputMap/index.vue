<template>
  <div>
    ...
    <el-dialog :visible.sync="box" append-to-body title="选择位置" width="80%" @close="handleClose">
      <div v-if="box">
        <el-input
          id="map__input"
        ...
        />
        <div>
          <div id="map__container" class="input-map-content-container" tabindex="0" />
          <div id="map__result" class="input-map-content-result" />
        </div>
      </div>
      <span slot="footer" class="dialog-footer">
        <el-button v-if="!disabled" icon="el-icon-check" type="primary" @click="handleSubmit"
          >确 定</el-button
        >
      </span>
    </el-dialog>
...
import MapLoader from '@/components/DuoJuHe/Form/form/InputMap/amap';
...
export default {
  ...
  data() {
    return {
      formattedAddress: '',
      address: this.value.length === 3 ? this.value[2] : '',
      poi: {},
      text: '',
      marker: null,
      map: null,
