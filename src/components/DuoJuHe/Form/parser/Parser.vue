...
import { evalExpression } from '@/utils/form/utils/expression';
import render from '@/components/DuoJuHe/Form/render/render';
import _ from 'lodash';
import { componentDefaultValue } from '@/components/DuoJuHe/Form/generator/config';
...
function formBtns(h) {
  ...
  let resetBtnStyle = {
    'background-color': formConfCopy.resetBtnColor,
    'border-color': formConfCopy.resetBtnColor,
  };
    ...
    return (
      <el-col>
        <el-form-item size="large">
          <el-row>
            <el-col span={this.span4} offset={this.offset6}>
              <el-button type="primary" style={style} onClick={this.submitForm}>
                {this.formConfCopy.submitBtnText}
              </el-button>
            </el-col>
            <el-col span={this.span9} offset={this.offset3}>
              <el-button style={resetBtnStyle} onClick={this.resetForm}>
                {this.formConfCopy.resetBtnText}
              </el-button>
            </el-col>
          </el-row>
        </el-form-item>
  ...
  if (this.formConfCopy.submitBtnText) {
    return (
      <el-col>
        <el-form-item size="large" class="submit-btn-form-item">
...
function setUpload(config, scheme, response, file) {
  ...
  let newValue = this[this.formConf.formModel][scheme.__vModel__];
  if (!newValue) {
    newValue = [];
  }
  ...
  //let fileListValue = []
  //fileListValue.push({name: file.name, url: response.data})

  this.$set(config, 'defaultValue', newValue);
  //this.$set(config, 'fileList', fileListValue)
