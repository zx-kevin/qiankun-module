<template>
  <el-header id="panel-header">
    <div class="module left-module">
      <span
        class="icon-badge"
        v-show="params.isRobot == 0"
        :class="{ 'red-color': params.listId == 1 }"
      >
        {{ params.listId == 1 ? '好友' : '群组' }}
      </span>
      <span class="nickname">{{ params.groupName }}</span>
      <span class="num">({{ groupNum }})</span>
    </div>

    <div v-show="params.listId == 1 && params.isRobot == 0" class="module center-module">
      <p class="online">
    ...
    <div class="module right-module">
      <el-tooltip content="历史消息" placement="top">
        <p>
          <el-icon @click="triggerEvent('history')"><Timer /></el-icon>
        </p>
      </el-tooltip>
      <el-tooltip content="群公告" placement="top">
        <p>
          <el-icon @click="triggerEvent('notice')"><Edit /></el-icon>
        </p>
      </el-tooltip>
      <el-tooltip content="群设置" placement="top">
        <p>
          <el-icon @click="triggerEvent('setting')"><Setting /></el-icon>
        </p>
      </el-tooltip>
    </div>
  </el-header>
