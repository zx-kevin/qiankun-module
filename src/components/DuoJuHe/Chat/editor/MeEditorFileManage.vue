<template>
  <el-container class="container animated bounceInUp" v-outside="closeBox" v-if="show">
    <el-header class="no-padding header" height="50px">
      <p>
        上传管理 <span v-show="total">({{ successNum }}/{{ total }})</span>
      </p>
      <el-icon @click="closeBox"><Close /></el-icon>
    </el-header>
    ...
    <el-main class="no-padding mian lum-scrollbar">
      ...
      <div v-for="file in items" v-show="!file.isDelete" :key="file.hashName" class="file-item">
        ...
        <div class="file-mian">
          <div class="progress">
            <el-progress type="dashboard" :percentage="file.progress" :width="50" :color="colors" />
        ...
        <div v-show="file.status == 2 || file.status == 3" class="file-means">
          ...
          <div v-show="file.status == 3" class="btns" @click="triggerUpload(file.hashName)">
            继续上传
          </div>
        </div>
      </div>
...
// Vue.use(ElProgress);
import { getBigFileUploadSplitInfo, bigFileSliceUpload } from '@/api/common/fileUpload';

import { formateSize, getFileExt, parseTime } from '@/utils/functions';
import { sendTalkFile } from '@/api/module/chat/im/chatTalkRecord';
  ...
  methods: {
    ...
    upload(file) {
      let fileType = file.name.split('.')[file.name.split('.').length - 1],
        file_arr = '.wmv,.mp4,.mov,.m4v,.avi,.dat,.mkv,.flv,.vob';
      ...
      chatFileApi(fileData).then((res) => {
        if (res && res.code === 200) {
          const { name: hashName, splitSize } = res.data;
          this.items.unshift({
            hashName: hashName,
            originalFile: file,
            filename: file.name,
            status: 2, // 文件上传状态 0:等待上传 1:上传中 2:上传完成 3:网络异常
            progress: 100,
          ...
          let data = res.data;
          data.chatType = file_arr.indexOf(fileType) > 0 ? 'video' : 'file';
        ...
        .then((res) => {
          if (res && res.code === 200) {
            ...
            if (this.items[$index].successNum == length) {
              ...
              if (res.data.isFileMerge) {
                let talkSend = {
                  hashName: res.data.hashName,
                  groupId: this.im_dialogue.groupId,
                  listId: this.im_dialogue.listId,
                };
                sendTalkFile(talkSend).then((res) => {
                  if (res && res.code === 200) {
                  } else {
