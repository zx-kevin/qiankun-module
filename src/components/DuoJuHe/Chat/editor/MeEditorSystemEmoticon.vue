  ...
  <div class="lum-dialog-mask">
    <el-container class="lum-dialog-box" v-outside="closeBox">
      ...
      <el-main class="no-padding mian lum-scrollbar">
        <ul>
          <li v-for="(item, i) in items" :key="item.itemId" class="no-select">
...
<script>
import {
  queryChatSystemEmoticonItemPageResList,
  saveChatEmoticonItem,
} from '@/api/module/chat/im/chatEmoticon';
...
export default {
  ...
  methods: {
    ...
    // 获取系统表情包列表
    loadSysEmoticon() {
      queryChatSystemEmoticonItemPageResList().then((res) => {
        this.items = res.data;
      });
    },
    ...
    setEmoticon(index, item, type) {
      saveChatEmoticonItem({
        emoticonId: item.itemId,
        type: type,
      }).then((res) => {
        if (type == 1) {
          this.items[index].status = 1;
          emoticon().APPEND_SYS_EMOTICON(res.data);
        } else {
          this.items[index].status = 0;
          emoticon().REMOVE_SYS_EMOTICON({
            emoticonId: item.itemId,
          });
        }
      });
