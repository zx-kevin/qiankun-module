  ...
  <div>
    <div class="user-card animated fadeIn">
      <div class="card-header">
        <img :src="$store.state.permission.visitCardBag" class="no-select" />
        <div class="user-avatar no-select">
          <img :src="$store.state.permission.headPortrait" :onerror="userStore.defaultAvatar" />
        </div>
        <div class="user-nickname">
          <i class="iconfont icon-qianming" />
          <span v-text="$store.state.permission.realName"></span>
        </div>
      </div>
      <div class="card-main">
        <div class="usersign">
          <div class="arrow"></div>
          <span v-if="$store.state.permission.motto">
            <span style="font-weight: 600">个性签名</span> ：
            {{ $store.state.permission.motto }}
          </span>
          <span v-else>
