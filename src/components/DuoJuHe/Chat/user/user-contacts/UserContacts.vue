  ...
  <div class="lum-dialog-mask">
    <el-container class="lum-dialog-box" v-outside="close">
        ...
        <el-container class="full-height">
          <el-aside width="250px" class="aside-border">
            <el-container class="full-height no-select">
              <el-header class="search-header" :class="{ shadow: headerShadow }" height="50px">
                <el-input
                  v-model="keywords"
                  clearable
                  size="small"
              ...
              <el-main class="no-padding">
                <el-scrollbar ref="scrollbar2" class="full-height" tag="section" :native="false">
                  <ul class="friend-items no-select">
                    <li v-for="item in search" :key="item.indexName" @click="triggerContacts(item)">
                      <el-avatar
                        class="avatar"
                      ...
                      <span class="nickname">
                        [{{ item.listId == 1 ? '友' : '群' }}] {{ item.realName }}
                      </span>
              ...
              <el-main>
                <el-scrollbar :native="false" tag="section" class="full-height">
                  <div class="selectd-items">
                    ...
                    >
                      <el-avatar :size="25" :src="item.headPortrait">
                        <img src="@/assets/images/chat/detault-avatar.jpg" />
                      </el-avatar>
                      <p>{{ item.realName }}</p>
                      <div class="triangle-topleft" :class="{ group: item.listId == 2 }"></div>
      ...
      <el-footer height="50px" class="footer">
        ...
        <el-button type="primary" size="small" :disabled="selected.length === 0" @click="confirm">
    ...
    keywords(val) {
      if (val == '') {
        ...
        this.search = this.contacts.filter((item) => {
          return item.realName.match(this.keywords) != null;
        });
      }
    },
    contacts(arr) {
  ...
  methods: {
    ...
    triggerContacts(item) {
      let index = this.contacts.findIndex((val) => {
        return val.indexName == item.indexName;
      });
    ...
    //加载好友列表
    loadFriends() {
      queryMyChatUsersFriendsPageResList({}).then((res) => {
        for (let o of res.data.records) {
          this.contacts.push({
            receiverId: o.friendId,
            listId: 1,
            indexName: `1_${o.friendId}`,
            realName: o.friendRemark ? o.friendRemark : o.realName,
    ...
    //加载群聊列表接口
    loadUserGroups() {
      getGroupPageApi({}).then((res) => {
        for (let o of res.data.records) {
          this.contacts.push({
            receiverId: o.groupId,
            listId: 2,
            indexName: `2_${o.groupId}`,
            realName: o.groupName,
            headPortrait: o.groupAvatar,
            checked: false,
          });
        }
      });
